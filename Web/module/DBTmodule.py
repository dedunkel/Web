...
# DB 생성
def createDB():
    conn = pymysql.connect(host='localhost', user='root', password='EKdms001^^', charset='utf8')
    cursor = conn.cursor()
...
def insertDB(writer, title, content):
    now = datetime.datetime.now()
    format = '%Y-%m-%d %H:%M:%S'
    now = now.strftime(format)
    print(now)
    sqlData = (now, writer, title, content)

    conn = pymysql.connect(host='localhost', user='root', password='EKdms001^^', db='Board', charset='utf8')
    cursor = conn.cursor()

    sql = "INSERT INTO Board_Table (idx, datetime, name, title, content) VALUES (NULL, %s, %s, %s, %s)"
